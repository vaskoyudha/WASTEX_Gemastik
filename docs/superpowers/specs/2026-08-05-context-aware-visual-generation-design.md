...
### 2b. Three-layer master prompt

The image endpoint accepts a single `prompt` string (no system/user role split), so the "how to correctly draw step images" instruction is embedded as a **fixed prefix** shared by every panel of every skill. Each panel's final prompt is built as three layers concatenated:

```text
[MASTER PROMPT — fixed, same for all panels]
...
view; composition and item must stay consistent across all panels that share
a reference image.

[REFERENCE POLICY — per panel]
...
[STEP-SPECIFIC — built from the skill]
<existing build_storyboard_prompt / before_after / mockup text>
```

This matches the manual `visuals/manual-generation/README.md` "Prompt Generator" block, now generated by the backend so every panel inherits it automatically.

If the provider/model lacks reference support, or no reference images are available, fall back to prompt-only (current behavior: master + step-specific, no reference policy lines).
...
- `test_prompt_has_three_layers` — generated prompt contains the master prompt prefix, the reference policy lines (when refs are passed), and the step-specific text.
